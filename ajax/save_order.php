...
try {
  ...
  // ✅ Nếu là địa chỉ đã lưu
  if (!empty($addr['saved_id'])) {
    // Lấy địa chỉ từ bảng user_addresses
    $stmt = $pdo->prepare("SELECT address_detail, ward, district, province FROM user_addresses WHERE address_id = ? AND user_id = ?");
    $stmt->execute([$addr['saved_id'], $user_id]);
