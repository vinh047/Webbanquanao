...
<?php else: ?>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <!-- Nút hủy đơn (chỉ khi chờ xác nhận) -->
          <?php if ($o['status'] === 'Chờ xác nhận'): ?>
            <form method="POST" action="/ajax/cancel_order.php" class="d-inline"
  ...
  <div class="d-flex justify-content-center align-items-center my-4">
    <!-- Về trang đầu -->
    <a 
      href="?page=donhang&page_order=1" 
      class="btn btn-outline-secondary<?= $page_order === 1 ? ' disabled' : '' ?>"
      <?= $page_order === 1 ? 'tabindex="-1" aria-disabled="true"' : '' ?>>
      &laquo;
    </a>

    <!-- Lùi một trang -->
    <a 
      href="?page=donhang&page_order=<?= max(1, $page_order - 1) ?>" 
      class="btn btn-outline-secondary ms-2<?= $page_order === 1 ? ' disabled' : '' ?>"
      <?= $page_order === 1 ? 'tabindex="-1" aria-disabled="true"' : '' ?>>
      &lsaquo;
    </a>

    <!-- Hiển thị số hiện tại / tổng -->
    <span class="border rounded mx-2 px-3 py-2">
      <?= $page_order ?> / <?= $totalPages ?>
    </span>

    <!-- Tiến một trang -->
    <a 
      href="?page=donhang&page_order=<?= min($totalPages, $page_order + 1) ?>" 
      class="btn btn-outline-secondary me-2<?= $page_order === $totalPages ? ' disabled' : '' ?>"
      <?= $page_order === $totalPages ? 'tabindex="-1" aria-disabled="true"' : '' ?>>
      &rsaquo;
    </a>

    <!-- Sang trang cuối -->
    <a 
      href="?page=donhang&page_order=<?= $totalPages ?>" 
      class="btn btn-outline-secondary<?= $page_order === $totalPages ? ' disabled' : '' ?>"
      <?= $page_order === $totalPages ? 'tabindex="-1" aria-disabled="true"' : '' ?>>
      &raquo;
    </a>
  </div>
<?php endif; ?>
  ...
  <!-- Ẩn sẵn content chi tiết để modal truy xuất -->
  <?php foreach ($orders as $o): ?>
    <?php
      $stmt2 = $pdo->prepare("
      ...
      <table class="table table-sm mb-0 text-center">
        <thead>
          <tr class="table-secondary">
            <th>Sản phẩm</th><th>Màu</th><th>Size</th>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <!-- Nội dung sẽ được JS load vào đây -->
        </div>
      </div>
    </div>
  </div>
  ...
  document.getElementById('orderDetailModal')
    .addEventListener('show.bs.modal', function(event) {
      ...
      const content = document.getElementById('detail-content-'+oid).innerHTML;
